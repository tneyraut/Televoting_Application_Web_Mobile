...
User::addQuery('USER_BY_ID', 'SELECT user_id,login,professeur,admin,annee FROM user WHERE user_id=:id');

User::addQuery('USER_CONNECT', 'SELECT user_id,login,admin,password,professeur,annee FROM user WHERE login=:login');
